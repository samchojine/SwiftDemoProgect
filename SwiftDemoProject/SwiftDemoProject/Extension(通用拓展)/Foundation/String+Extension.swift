...
extension String{
    ...
    /// 正则验证
    /// - Parameter predicate: 正则表达式
    func JYNSPredicate(predicate:String) -> Bool{
        let pre = NSPredicate(format:"SELF MATCHES %@", predicate)
        return pre.evaluate(with: self)
    }
    
    /// 判断是否拥有数字和字母
    func JYVerificationNumberAndLetter() -> Bool{
        do {
    ...
    /// 添加价格富文本 就是￥200.00这种样式 只有前面的符号字体变小的样式
    /// - Parameter font: 符号的字体大小
    func addMoneyAttributedString(font:CGFloat)->NSMutableAttributedString{
        let attstr = NSMutableAttributedString(string: self)
        attstr.addAttribute(NSAttributedString.Key.font, value: UIFont.systemFont(ofSize: font), range: NSRange(location: 0, length: 1))
        return attstr
    }
    
    /// 计算文字的size
    /// - Parameter str: 要计算的文字
    /// - Parameter font: 字体大小
    /// - Parameter width: 最大的宽度
    ...
    func klc_calculateWidthWithFont(_ font:UIFont) -> CGFloat {
    ...
    func klc_calculateWidthWithFontSize(_ fontSize:CGFloat) -> CGFloat{
        let font = UIFont.systemFont(ofSize: fontSize)
        return self.klc_calculateWidthWithFont(font)
    }
    
    func klc_calculateHeightWithFontSize(_ fontSize:CGFloat,width:CGFloat) -> CGFloat {
        let size = (self as NSString).boundingRect(with: CGSize(width: width, height: CGFloat.greatestFiniteMagnitude), options: [.usesLineFragmentOrigin], attributes: [NSAttributedString.Key.font:UIFont.systemFont(ofSize: fontSize)], context: nil)
        return size.height
    }
